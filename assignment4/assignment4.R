...
print_row_type2 <- function(language, model0, model1, model2, model3, model4, model5, model1p, model2p, model3p, model4p, model5p){
  sprintf(r"( %s & %.3f & %.3f & %.3f & %.3f & %.3f & %.3f & %.3f & %.3f & %.3f & %.3f & %.3f \\)",
          language,
          model0,  # Model 0
          model1,  # Model 1
          model2,  # Model 2
...
for(language in languages){
  ...
  # ---------------------------------------------------------------------------# 
  # Params giving the best fit for the model
  param_a_model5 <- coef(model_5)["a"]
  param_b_model5 <- coef(model_5)["b"]
  param_c_model5 <- coef(model_5)["c"]
  # Errors of the non-linear regression model
  s_model5 <- sqrt(deviance(model_5)/df.residual(model_5))
  s_list <- append(s_list, s_model5)
  cat("Residual Standard Error (s): \n")
  print(s_model5)
  aic_model5 <- AIC(model_5)
  AIC_list <- append(AIC_list, aic_model5)
  cat("AIC: \n")
  print(aic_model5)
  ...
  if(model_5p$convInfo["stopCode"] != 0 && model_5p$convInfo["stopCode"] != 3) {
    signalCondition(model_5p$convInfo["stopMessage"])
  }
  models[["model_5p"]] <- model_5p
  # ---------------------------------------------------------------------------# 
  # Params giving the best fit for the model
  param_a_model5p <- coef(model_5p)["a"]
  param_b_model5p <- coef(model_5p)["b"]
  param_c_model5p <- coef(model_5p)["c"]
  param_d_model5p <- coef(model_5p)["d"]
  # Errors of the non-linear regression model
  s_model5p <- sqrt(deviance(model_5p)/df.residual(model_5p))
  s_list <- append(s_list, s_model5p)
  cat("Residual Standard Error (s): \n")
  print(s_model5p)
  aic_model5p <- AIC(model_5p)
  AIC_list <- append(AIC_list, aic_model5p)
  cat("AIC: \n")
  print(aic_model5p)
  ...
  lines(data$vertices, (data$vertices + 1)/3, type="l", lty=1, col="grey")
  ...
  table_params <- paste(table_params, sprintf(r"( %s & \multicolumn{1}{l|}{%.3f} & %.3f & \multicolumn{1}{l|}{%.3f} & %.3f & \multicolumn{1}{l|}{%.3f} & \multicolumn{1}{l|}{%.3f} & %.3f & %.3f & \multicolumn{1}{l|}{%.3f} & %.3f & \multicolumn{1}{l|}{%.3f} & %.3f & %.3f & \multicolumn{1}{l|}{%.3f} & %.3f & %.3f & \multicolumn{1}{l|}{%.3f} & %.3f & %.3f  & %.3f & %.3f & %.3f & \multicolumn{1}{l|}{%.3f}\\)",
                                              ...
                                              param_a_model4,
                                              param_a_model5, param_b_model5, param_c_model5,
                                              ...
                                              param_a_model5p, param_b_model5p, param_c_model5p, param_d_model5p
  ), sep="\n")
}
