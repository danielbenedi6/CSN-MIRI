...
fit_degree_sequence <- function(degree_sequence) {
  ...
  N <- sum(degree_sequence$f)      # Number of nodes
  k_max <- max(degree_sequence$k)  # Maximum degree #TODO: do we use k_max later?
  M <- sum(degree_sequence$f * degree_sequence$k)            # Total cumulative degree/Sum of nodes
  x <- degree_sequence             # The values themselves
  M_prime = sum(degree_sequence$f * log(degree_sequence$k))  # Logarithmic cumulative degree/Logarithmic sum of nodes
...
```{r}
degree_spectrum <- table(degsec_BA)
...
best_func <- switch(
  best_name,
  "poisson" = function(k) attributes(summary(best_model))$coef[1]**k * exp(-attributes(summary(best_model))$coef[1]) / factorial(k) / ( 1 - exp(-attributes(summary(best_model))$coef[1])),
  "geometric" = function(k) (1-attributes(summary(best_model))$coef[1])**k*attributes(summary(best_model))$coef[1],
  "zetafixed" = function(k) k**(-3)/zeta(3),
  "zeta" = function(k) k**(-attributes(summary(best_model))$coef[1])/zeta(attributes(summary(best_model))$coef[1]),
  "rightzeta" = function(k) k**(-attributes(summary(best_model))$coef[1])/ sum( (1:km)^(-attributes(summary(best_model))$coef[1])),
  "altmann" = function(k) k**(-attributes(summary(best_model))$coef[1])*exp(-attributes(summary(best_model))$coef[2]*k) / sum( (1:sum(ds$f)) **(-attributes(summary(best_model))$coef[1])*exp(-(1:sum(ds$f))*attributes(summary(best_model))$coef[2] ) ),
)
```

```{r}
matplot(ds$k,ds$p, main=sprintf("Best model with Barbási-Albert: %s", best_name),
          xlab = "degree", ylab = "number of vertices" , log = "xy" , pch = 19)
lines(1:k_max, best_func(1:k_max))
```

```{r}
deg_spec <- read.csv('data/full_ba_deg_seq.txt.mean', sep=" ", header=FALSE)
colnames(deg_spec) = c("k","f")
for( j in 1:length(deg_spec) ){
  #Yes, this is weird, but how R manages factors is weirder
  deg_spec$k[j] = as.numeric(as.character(deg_spec$k[j])) 
}
deg_spec$p = deg_spec$f/sum(deg_spec$f)
...
lines( 1:k_max , 
       poisson(1:k_max , attributes(summary(ds_results$mles[["poisson"]]))$coef[1]),
       col = 2)
lines( 1:k_max , 
       geometric(1:k_max , attributes(summary(ds_results$mles[["geometric"]]))$coef[1]) ,
       col = 3)
lines( 1:k_max ,
       zeta_3(1:k_max) ,
       col = 4 )
lines( 1:k_max ,
       zeta_g(1:k_max , attributes(summary(ds_results$mles[["zeta"]]))$coef[1]) ,
       col = 5)
lines( 1:k_max , 
       right_truncated_zeta( 
          1:k_max , 
          attributes(summary(ds_results$mles[["rightzeta"]]))$coef[1], 
          km) ,
       col = 6)
lines( 1:k_max , 
       altmann(
         1:k_max , 
         sum(ds$f) , 
         attributes(summary(ds_results$mles[["altmann"]]))$coef[1], 
         attributes(summary(ds_results$mles[["altmann"]]))$coef[2]) ,
       col = 7)
legend( 10**(0.45*(log10(max(ds$k))+log10(min(ds$k)))) , max(ds$p) , 
        legend = c(TeX(sprintf(r'(D. Poisson($\lambda = %0.3f$))', attributes(summary(ds_results$mles[["poisson"]]))$coef[1])), 
                   TeX(sprintf(r'(D. Geom($\q = %0.3f$))', attributes(summary(ds_results$mles[["geometric"]]))$coef[1])), 
                   "Zeta(3)", 
                   TeX(sprintf(r'(Zeta($\gamma = %0.3f$))', attributes(summary(ds_results$mles[["zeta"]]))$coef[1])),
                   TeX(sprintf(r'(Trunc Zeta($\gamma = %0.3f, k_{max} = %f$))', attributes(summary(ds_results$mles[["rightzeta"]]))$coef[1], km)),
                   TeX(sprintf(r'(Altmann($\gamma = %0.3f, \delta = %0.3f$))', attributes(summary(ds_results$mles[["altmann"]]))$coef[1], attributes(summary(ds_results$mles[["altmann"]]))$coef[2]))
        ), 
